...
def build_similar(db: sqlite3.Connection) -> None:
    ...
    """)
    looks = [(c, o, "look", r, round(s, 4), None) for c, o, r, s in similar.lookalikes(db)]
    db.executemany("INSERT INTO similar VALUES (?,?,?,?,?,?)", looks)
    means = list(similar.meaning_neighbours(db))
