...
| **Semantic search** | When a longer query finds nothing in the dictionary ("the feeling of missing the old days", "which kanji for the side of a box"), a signed-in user's query goes to a language model (GPT-6 Luna, through OpenRouter). It suggests kanji and words, most relevant first, and a note where the choice needs explaining; a kanji described by its parts is checked against the decomposition graph. Only what the dictionary has is shown, under a *Semantic search* label. `tests/semantic_eval.py` is how the model was chosen. Needs `OPENROUTER_API_KEY`; without it the feature stays hidden. |
...
| **Drawing** | Excalidraw in the app's colours, with picture tools it lacks: lasso, box and magic-wand selection of a picture's pixels, and *select the subject* (BRIA RMBG-1.4 run on the device -- on WebGPU at full size, otherwise on the CPU at whatever size it manages in about two seconds; 44 MB fetched on first use, built by `pipeline/rmbg_model.py` before `npm run build`) -- then erase, keep only, cut out or copy out. Pictures can be searched on Pixabay from inside the editor; needs `PIXABAY_API_KEY`. |
