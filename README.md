...
| **Semantic search** | When a longer query finds nothing in the dictionary ("the feeling of missing the old days", "which kanji for the side of a box"), a signed-in user's query goes to Claude Sonnet 5 through OpenRouter. It suggests kanji and words, most relevant first, and a note where the choice needs explaining. Only what the dictionary has is shown, under a label saying where it came from. Needs `OPENROUTER_API_KEY`; without it the feature stays hidden. |
