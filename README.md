...
| **Drawing** | Excalidraw in the app's colours, with picture tools it lacks: lasso, box and magic-wand selection of a picture's pixels, and *select the subject* (BRIA RMBG-1.4, run on the device, 44 MB fetched on first use) -- then erase, keep only, cut out or copy out. Pictures can be searched on Pixabay from inside the editor; needs `PIXABAY_API_KEY`. |
